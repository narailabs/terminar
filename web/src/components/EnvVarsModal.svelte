<script lang="ts">
  import EnvVarEditorModal from './EnvVarEditorModal.svelte';
  import { getSessionsContext } from '../lib/sessionContext.svelte';
  import {
    globalEnvVars,
    addEnvVar,
    updateEnvVar,
  ...
  function getScopeLabel(): string {
    ...
    const session = sessionsBox.value.find(s => s.id === selectedScope);
    return session?.name || selectedScope.slice(0, 8);
  }

  function getSessionDisplayName(session: { id: string; name: string }): string {
    ...
    <div class="modal-body">
      <nav class="scope-nav">
        ...
        {#if sessionsBox.value.length > 0}
          ...
          {#each sessionsBox.value as session (session.id)}
            <button
              class="nav-item"
...
<style>
  ...
  .modal {
    background: var(--ui-bg-secondary, #1e1f22);
  ...
  .modal-header {
    ...
    border-bottom: 1px solid var(--ui-border, #242629);
    flex-shrink: 0;
  }
  ...
  .scope-nav {
    width: 180px;
    flex-shrink: 0;
    border-right: 1px solid var(--ui-border, #242629);
    padding: 0.5rem;
    overflow-y: auto;
    display: flex;
    flex-direction: column;
  ...
  .nav-item {
    ...
    border-radius: 6px;
    background: none;
    color: var(--ui-text-secondary, #a6adc8);
    font-size: 0.8rem;
    cursor: pointer;
    text-align: left;
    width: 100%;
  ...
  .badge {
    background: var(--ui-bg-tertiary, #47484a);
    color: var(--ui-text-secondary, #a6adc8);
    font-size: 0.65rem;
  ...
  .nav-divider {
    height: 1px;
    background: var(--ui-border, #242629);
    margin: 0.35rem 0;
  }
