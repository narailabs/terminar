...
test('creates and switches sessions - basic flow', async ({ page }) => {
  ...
  await expect(page.locator('.xterm-rows').first()).not.toContainText(marker1, { timeout: 5000 });
});
...
test.skip('creates and switches sessions - history preservation', async ({ page }) => {
  ...
  await expect(page.locator('.xterm-rows').first()).toContainText(marker1, { timeout: 5000 });
  await expect(page.locator('.xterm-rows').first()).not.toContainText(marker2);
});
...
test('sessions persist on page refresh', async ({ page }) => {
  ...
  await expect(page.locator('.xterm-rows').first()).toContainText(marker, { timeout: 10000 });
});
