...
test.describe('Shell Commands', () => {
  ...
  test('can run pwd', async ({ page }) => {
    ...
    await expect(page.locator('.xterm-rows')).toContainText('/Users', { timeout: 5000 });
  });
  ...
  test('can run ls', async ({ page }) => {
    ...
    await expect(page.locator('.xterm-rows')).toContainText(/total|drwx/, { timeout: 5000 });
  });
  ...
  test('can run command with arguments', async ({ page }) => {
    ...
    await page.keyboard.press('Enter');
    await expect(page.locator('.xterm-rows')).toContainText('test argument', { timeout: 5000 });
  });
