...
async fn send_list_sessions_and_get_response(
    ...
    while start.elapsed() < Duration::from_secs(timeout_secs) {
        if let Some(Ok(Message::Text(text))) = reader.next().await
            && let Ok(ServerMessage::SessionList { sessions }) =
...
async fn test_auth_required_non_auth_first_message() {
                ...
                if let Ok(msg) = serde_json::from_str::<ServerMessage>(&text) {
                    match msg {
                        ...
                        ServerMessage::SessionList { .. } => {
                            panic!(
                                "Server should NOT return SessionList without authentication when require_auth is set"
                            );
                        }
                        _ => {}
                    }
    ...
    assert!(
        got_auth_error,
        "Server should require authentication and reject wrong token with AUTH_FAILED"
    );
}
...
async fn test_auth_invalid_token_rejected() {
    ...
    assert!(
        got_auth_error,
        "Server should reject invalid token with AUTH_FAILED error"
    );
}
...
async fn test_auth_empty_token() {
    ...
    assert!(
        got_auth_error,
        "Server should reject empty token with AUTH_FAILED error"
    );
}
...
async fn test_wrong_field_types() {
    ...
    let wrong_type_msg =
        r#"{"type":"create_session","cwd":"/","shell":"bash","env":{},"cols":"eighty","rows":24}"#;
    sender
        .send(Message::Text(wrong_type_msg.to_string()))
        .await
        .unwrap();
...
async fn test_cwd_path_traversal() {
    ...
    while start.elapsed() < Duration::from_secs(3) {
        if let Some(Ok(Message::Text(text))) = reader.next().await
            && let Ok(ServerMessage::Error { message, .. }) =
                serde_json::from_str::<ServerMessage>(&text)
        {
            assert!(
...
async fn test_cwd_dot_dot_embedded() {
    ...
    assert!(
        got_error,
        "Server should reject cwd with embedded '..' path traversal"
    );
...
async fn test_ld_preload_filtered() {
    ...
    assert!(
        session_created,
        "Server should create the session after filtering LD_PRELOAD"
    );
...
async fn test_dyld_insert_libraries_filtered() {
    ...
    assert!(
        session_created,
        "Server should create the session after filtering DYLD_* vars"
    );
...
async fn test_pair_exchange_rate_limiting() {
    ...
    assert!(
        got_rate_limited,
        "Should be rate limited after multiple invalid pairing attempts"
    );
}
...
async fn test_ws_auth_rate_limit_after_5_failures() {
    ...
    assert!(
        got_rate_limited,
        "Server should send RATE_LIMIT_EXCEEDED after {} failed attempts",
        auth_failed_count
    );
    assert!(
        auth_failed_count <= 5,
        "Should get at most 5 AUTH_FAILED before RATE_LIMIT_EXCEEDED, got {}",
        auth_failed_count
    );
}
...
async fn test_ws_auth_allows_retry_after_failure() {
    ...
    socket
        .send(Message::Text(serde_json::to_string(&bad_auth).unwrap()))
        .await
        .unwrap();
    ...
    while start.elapsed() < Duration::from_secs(2) {
        match socket.next().await {
            Some(Ok(Message::Text(text))) => {
                if let Ok(ServerMessage::Error { error_code, .. }) =
                    serde_json::from_str::<ServerMessage>(&text)
                {
                    assert_eq!(
                        error_code.as_deref(),
    ...
    assert!(
        got_first_error,
        "Should get AUTH_FAILED for first bad token"
    );
    ...
    let send_result = socket
        .send(Message::Text(serde_json::to_string(&second_auth).unwrap()))
        .await;
    assert!(
        send_result.is_ok(),
        "Connection should still be open for retry after first AUTH_FAILED"
    );
    ...
    assert!(
        got_second_error,
        "Should get AUTH_FAILED for second bad token (connection allows retry)"
    );
}
